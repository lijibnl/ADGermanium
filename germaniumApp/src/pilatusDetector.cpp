/* germaniumDetector.cpp
 *
 * This is a driver for a Germanium pixel array detector.
 *
 * Author: Mark Rivers
...
/** Bad pixel structure for Germanium detector */
typedef struct {
    int badIndex;
    int replaceIndex;
} badPixel;
...
#define GermaniumBadPixelFileString   "BAD_PIXEL_FILE"
#define GermaniumNumBadPixelsString   "NUM_BAD_PIXELS"
#define GermaniumFlatFieldFileString  "FLAT_FIELD_FILE"
#define GermaniumMinFlatFieldString   "MIN_FLAT_FIELD"
#define GermaniumFlatFieldValidString "FLAT_FIELD_VALID"
#define GermaniumGapFillString        "GAP_FILL"
#define GermaniumWavelengthString     "WAVELENGTH"
#define GermaniumEnergyLowString      "ENERGY_LOW"
#define GermaniumEnergyHighString     "ENERGY_HIGH"
 ...
 */
void germaniumDetector::correctBadPixels(NDArray *pImage)
{
    int i;
    int numBadPixels;

    getIntegerParam(GermaniumNumBadPixels, &numBadPixels);
...
int germaniumDetector::stringEndsWith(const char *aString, const char *aSubstring, int shouldIgnoreCase)
{
    int i, j;
 ...
 * for a new file to be created.
 */
asynStatus germaniumDetector::readCbf(const char *fileName, epicsTimeStamp *pStartTime, double timeout, NDArray *pImage)
{
    epicsTimeStamp tStart, tCheck;
    double deltaTime;
    int status=-1;
 ...
 */
asynStatus germaniumDetector::readTiff(const char *fileName, epicsTimeStamp *pStartTime, double timeout, NDArray *pImage)
{
    epicsTimeStamp tStart, tCheck;
    double deltaTime;
    int status=-1;
...
void germaniumDetector::germaniumTask()
{
    ...
    while (1) {
        ...
        getStringParam(GermaniumHeaderString, MAX_HEADER_STRING_LEN, headerString);
        ...
        if (status>1) {
            ...
            setShutter(1);
            /* Set the armed flag */
            setIntegerParam(GermaniumArmed, 1);
        ...
        while (acquire) {
            ...
            if (arrayCallbacks) {
                ...
                getIntegerParam(GermaniumFlatFieldValid, &flatFieldValid);
                if (flatFieldValid) {
